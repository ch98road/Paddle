...
void Tensor::CopyFromCpu(const T *data) {
  ...
  if (place_ == PlaceType::kCPU) {
    ...
    VLOG(4) << "ch -- t_data ptr is: " << t_data;
    VLOG(4) << "ch -- data ptr is: " << data;
    for (int i = 0; i < tensor->numel(); i++) {
      VLOG(4) << "ch -- CopyToCpuImpl data[" << i << "] is: " << data[i];
    }
    ...
    paddle::memory::Copy(gpu_place,
                         ...
                         ele_size,
                         dev_ctx->stream());
    VLOG(4) << "ch -- CopyFromCpu xpu t_data ptr is: " << t_data;
    VLOG(4) << "ch -- CopyFromCpu cpu data ptr is: " << data;
#else
    PADDLE_THROW(paddle::platform::errors::Unavailable(
    ...
    VLOG(4) << "ch -- CopyFromCpu cpu data ptr is: " << data;
    // for (int i = 0; i < tensor->numel(); i++) {
    //   VLOG(4) << "ch -- CopyFromCpu data[" << i << "] is: " << data[i];
    // }
#else
    PADDLE_THROW(paddle::platform::errors::Unavailable(
...
void Tensor::CopyToCpuImpl(T *data,
  ...
  if (paddle::platform::is_cpu_place(t_place)) {
    ...
    VLOG(4) << "ch -- CopyToCpuImpl ipu place";
...
#if defined(PADDLE_WITH_CUDA) || defined(PADDLE_WITH_HIP)
    VLOG(4) << "ch -- CopyToCpuImpl gpu place";
...
#ifdef PADDLE_WITH_XPU
    for (int i = 0; i < 20; i++) {
      VLOG(4) << "ch -- before CopyToCpuImpl data[" << i << "] is: " << data[i];
    }
    ...
    paddle::memory::Copy(paddle::platform::CPUPlace(),
                         ...
                         t_data,
                         ele_num * sizeof(T));
    VLOG(4) << "ch -- CopyToCpuImpl cpu data ptr is: " << data;
    VLOG(4) << "ch -- CopyToCpuImpl xpu t_data ptr is: " << t_data;
    for (int i = 0; i < 20; i++) {
      VLOG(4) << "ch -- CopyToCpuImpl data[" << i << "] is: " << data[i];
    }
...
#ifdef PADDLE_WITH_CUSTOM_DEVICE
    VLOG(4) << "ch -- CopyToCpuImpl custom device";
    paddle::platform::DeviceContextPool &pool =
        paddle::platform::DeviceContextPool::Instance();
...
#ifdef PADDLE_WITH_ONNXRUNTIME
  VLOG(3) << "ch -- PADDLE_WITH_ONNXRUNTIME ";
  if (is_ort_tensor_) {
    std::vector<int> shape;
    // input handle
    if (idx_ < 0) {
